"""
train_gpt.py — Parameter Golf submission.
Credits: modded-nanogpt baseline, @raahilshah #162, @notapplica #60, @mattqlf #50.
Novel: adaptive softcap (learned per-position logit scaling).
"""
...
class Hyperparameters:
    ...
    muon_beta2 = float(os.environ.get("MUON_BETA2", 0.0))  # 0=Muon, >0=Mousse-lite diagonal preconditioning
    ema_decay = float(os.environ.get("EMA_DECAY", 0.0))  # 0 = disabled, ~0.997 typical
    late_qat_threshold = float(os.environ.get("LATE_QAT_THRESHOLD", 0.0))  # 0 = disabled, ~0.15 typical
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 0))  # 0 = disabled, ~8192 typical
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,vr_lambda,softcap_proj",
    ).split(",")
    if pattern
)
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        # GPTQ-lite: search over clipping percentiles to minimize reconstruction error.
        # Credit: @signalrush PR #414, @thwu1 PR #379.
...
def quantize_int6_per_row(t: Tensor) -> tuple[Tensor, Tensor]:
    """Int6 per-row quantization with GPTQ-lite search. Credit: @nanlliu #39, @signalrush #414."""
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float("inf")
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], use_int6: bool = False, num_layers: int = 0):
    # Export format: per-row int8 (or mixed int6/int8 for large 2D float tensors).
    ...
    for name, tensor in state_dict.items():
        ...
        # Int6 for block weights (attn+MLP), int8 for embedding
        should_int6 = use_int6 and t.ndim == 2 and "blocks." in name
        if should_int6:
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        if scheme in ("per_row", "per_row_int6") or s.ndim > 0:
...
class CastedLinear(nn.Linear):
    ...
    # Late QAT: simulate int6 quantization noise via STE when enabled.
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # Value Residual: preserve token identity through depth (credit: ResFormer, PR #413)
        self.vr_lambda = nn.Parameter(torch.tensor([0.5, 0.5], dtype=torch.float32))
        # Sparse Attention Gate: per-head gating from first 12 dims of residual (credit: modded-nanogpt #117)
        self.sparse_gate_w = nn.Parameter(torch.zeros(num_heads, 12, dtype=torch.float32))

    def forward(self, x: Tensor, v0: Tensor | None = None) -> tuple[Tensor, Tensor]:
        bsz, seqlen, dim = x.shape
        gate = torch.sigmoid(F.linear(x[..., :12], self.sparse_gate_w.to(dtype=x.dtype)))  # (B, T, H)
        ...
        y = y * gate.transpose(1, 2).unsqueeze(-1)  # sparse gating
        y = y.transpose(1, 2).contiguous().reshape(bsz, seqlen, dim)
        return self.proj(y), raw_v


class BigramHashEmbedding(nn.Module):
    """Bigram context via hashing. Credit: @unnir PR #102, #135."""
...
class GPT(nn.Module):
    def __init__(
        ...
        # Adaptive softcap: learned per-position logit scaling (novel technique)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Hourglass: mean-pool between encoder/decoder for regularization
        ...
        cap = self.logit_softcap + self.softcap_proj(x)
...
def _forward_logits(model: GPT, input_ids: Tensor) -> Tensor:
    """Forward pass returning logits (no loss). Used by sliding window eval."""
    ...
    for i in range(model.num_encoder_layers):
        x, raw_v = model.blocks[i](x, x0, v0=v0)
        if v0 is None:
            v0 = raw_v
        skips.append(x)
    if model.group_size > 1:
        B, S, D = x.shape
        x = x.reshape(B, S // model.group_size, model.group_size, D).mean(dim=2)
        x = torch.cat([torch.zeros_like(x[:, :1]), x[:, :-1]], dim=1)
        x = x.unsqueeze(2).expand(B, S // model.group_size, model.group_size, D).reshape(B, S, D)
    for i in range(model.num_decoder_layers):
        if skips:
            x = x + model.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
        x, _ = model.blocks[model.num_encoder_layers + i](x, x0, v0=v0)
    ...
    cap = model.logit_softcap + model.softcap_proj(x_flat)
    cap = cap.clamp(min=5.0)
    return (cap * torch.tanh(logits_proj.reshape(-1, logits_proj.size(-1)) / cap)).reshape(logits_proj.shape)
...
def eval_val_sliding(
    ...
    """Sliding window evaluation. Credit: @mattqlf PR #50."""
    ...
    base_model.eval()
    compiled_logits = torch.compile(_forward_logits, dynamic=False, fullgraph=True)
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                model_fn = base_model.module if hasattr(base_model, 'module') else base_model
                logits = compiled_logits(model_fn, x_batch)
...
def main() -> None:
    ...
    while True:
        ...
        # Late QAT: enable int6 quantization noise when entering warmdown
    ...
    quant_obj, quant_stats = quantize_state_dict_int8(base_model.state_dict(), use_int6=args.use_int6, num_layers=args.num_layers)
    quant_label = "int6" if args.use_int6 else "int8"
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model {quant_label}+{_COMPRESSOR}: {quant_file_bytes} bytes")
    ...
    # Sliding window eval (credit: @mattqlf PR #50)
    if args.eval_stride > 0:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_sliding_window val_loss:{sw_loss:.4f} val_bpb:{sw_bpb:.4f} "

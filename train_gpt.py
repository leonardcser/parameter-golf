...
# FA3 not available on this GPU — use PyTorch SDPA instead
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,softcap_proj",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Transpose (B,S,H,D) -> (B,H,S,D) for SDPA
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v_t = v.transpose(1, 2)
        y = F.scaled_dot_product_attention(
            q, k, v_t, attn_mask=None, is_causal=True,
            enable_gqa=(self.num_kv_heads != self.num_heads),
        )
        # Back to (B,S,H,D) for XSA
        y = y.transpose(1, 2)
        if self.use_xsa:
...
class GPT(nn.Module):
    def __init__(
        ...
        # Adaptive softcap: learned per-position logit scaling (novel contribution)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        cap = self.logit_softcap + self.softcap_proj(x_flat)
